...
def create_and_send_newsletter():
    ...
    # 2. 미디어 총평 및 에디터 코멘트 수집 (새로 추가된 news-new.py)
    try:
        news_result = subprocess.run(["python3", "news-new.py"], capture_output=True, text=True, check=True)
        # 디스코드에 전송될 때 지저분해 보이지 않도록 news-new.py 내부의 로딩 텍스트(⏳...)는 제거하고 핵심만 담습니다.
        news_text = news_result.stdout.replace("⏳ '두산베어스' 최신 기사 10개를 수집하고 요약을 생성하는 중입니다...", "").strip()
    except subprocess.CalledProcessError as e:
        news_text = f"미디어 총평 크래시 에러:\n{e.stderr.strip() if e.stderr else e}"
    except Exception as e:
        news_text = f"미디어 총평을 불러오지 못했습니다: {e}"
    ...
    if webhook_url:
        ...
        print("\n⚠️ 디스코드 웹훅 URL이 설정되지 않아 텍스트만 출력합니다.")
